    ...
    for c in entry.chars() {
        match c {
            ...
            x if !x.is_whitespace() && space => {
                ...
                actual_string = x.to_string();
            }
            x if x.is_whitespace() && !dquote && !squote && !bslash => space = true,
            x if x.is_whitespace() && bslash => {
                bslash = false;
                actual_string.push(x);
            }
            x if (x == '\'' || x == '"' || x == '\\') && bslash => {
                bslash = false;
...
fn default_env() -> MaybeArray<EnvVar> {
    MaybeArray::Multiple(vec![])
}
fn default_alone_zero() -> MaybeArray<i32> {
    MaybeArray::Alone(0)
}
...
fn to_bytes(input: Vec<String>) -> Vec<Vec<*const c_char>> {
    input
        .into_iter()
        .map(|e| {
            let mut tmp = e.into_bytes();
            tmp.push(b'\0');
            tmp.iter().map(|b| *b as *const c_char).collect()
        })
        .collect()
}
