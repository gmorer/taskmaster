...
type Clients = Mutex<HashMap<usize, server::Client>>;
...
fn main() -> Result<(), Error> {
    ...
    let (listening_waker, mut emiting_waker) = Waker::create()?;
    ...
    let clients: Arc<Clients> = Arc::new(Mutex::new(std::collections::HashMap::new()));
    ...
    loop {
        match rx.recv() {
            Ok(ev) => execut(&ev, &mut config, &mut started, &*clients, &mut emiting_waker),
